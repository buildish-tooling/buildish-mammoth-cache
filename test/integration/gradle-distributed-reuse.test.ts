...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.match(aggregatorLog, /resolved workerA: guava-33\.6\.0-jre\.jar/);
    assert.match(aggregatorLog, /resolved workerB: commons-io-2\.22\.0\.jar/);
    assert.ok(
      !/(?:Downloading|Downloaded).*(?:guava-33\.6\.0-jre|commons-io-2\.22\.0)\.jar/.test(
        aggregatorLog,
      ),
...
/**
 * Vitest entry point for the Gradle distributed-reuse integration test.
 *
 * Runs the full worker-A → worker-B → aggregator delta-exchange flow end-to-end using a local
 * {@link FakeArtifactApi} for artifact exchange. Requires Java 21+ and a POSIX shell (the
 * fixture only ships a `gradlew` script). The test timeout is set to 10 minutes to accommodate
 * Gradle's initial download and dependency resolution.
 *
 ...
 */
test('Gradle distributed-reuse integration', main, 600_000);
